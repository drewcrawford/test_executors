...
unavailable there; [`spawn_on`] needs a thread `std` cannot spawn on that
target; and [`spin_on`] never yields to the event loop, so anything waiting on
it is waiting forever. Use them on native, or from a Web Worker.
